...
run_dataset_benchmark() {
  ...
  local run_root="${RESULTS_ROOT}/${dataset}/${RUN_STAMP}"
  ...
  print_progress "${CURRENT_STEP}" "${TOTAL_STEPS}" "running ${dataset} -> ${run_root} (experiments ${EXP_START_INDEX}-${EXP_END_INDEX})"
  "${cmd[@]}"
}
...
for dataset in "${DATASETS[@]}"; do
  ...
  RUN_ROOTS+=("$(latest_run_root "${dataset}")")
done

COMPARISON_OUTPUT_DIR="${RESULTS_ROOT}/comparisons/${RUN_STAMP}"
